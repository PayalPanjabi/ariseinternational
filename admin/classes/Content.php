...
Class Content extends DBConnection {
	...
	public function dahihandi(){
		...
		if(empty($id)){
			$sql ="INSERT INTO dahihandi set $data";
		}else{
			$sql ="UPDATE dahihandi set $data where id = {$id}";
		}
		$save = $this->conn->query($sql);
		$action = empty($id) ? "added":"updated";
		if($save){
			...
			$resp['status']='success';
			$resp['message']= " dahihandi Details successfully ".$action;
			$this->settings->set_flashdata('success',$resp['message']);
	...
	public function delete_dahihandi(){
		extract($_POST);
		$fpath = $this->conn->query("SELECT file_path FROM dahihandi where id = $id")->fetch_array()['file_path'];
		$qry = $this->conn->query("DELETE FROM dahihandi where id = $id");
		if($qry){
			if(is_file(base_app.$fpath))
					unlink(base_app.$fpath);
			$resp['status']='success';
			$resp['message']= " dahihandi Details successfully deleted";
			$this->settings->set_flashdata('success',$resp['message']);
		}else{
			$resp['status']='Failed';
			$resp['error']= $this->conn->error;
...
switch ($action) {
	...
	break;
	case 'dahihandi':
		echo $Content->dahihandi();
	break;
	case 'service_delete':
		echo $Content->service_delete();
	break;
	case 'delete_dahihandi':
		echo $Content->delete_dahihandi();
	break;
	case 'testimonial':
