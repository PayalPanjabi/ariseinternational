...
window.history.pushState(newState,'')},_removeQueryField=function(query,key){var newQuery=query;if(newQuery){var keyRegex1=new RegExp('\\?'+key+'=[^&]*');var keyRegex2=new RegExp('&'+key+'=[^&]*');newQuery=newQuery.replace(keyRegex1,'?');newQuery=newQuery.replace(keyRegex2,'')}
return newQuery},_pushQuitToHistory=function(){if(!hasHistorySupport||!options.history){return}
var newQuery=_removeQueryField(document.location.search,'imageLightboxIndex');newQuery=_removeQueryField(newQuery,'imageLightboxSet');window.history.pushState({},'')},_getQueryField=function(key){var keyValuePair=new RegExp('[?&]'+key+'(=([^&#]*)|&|#|$)').exec(document.location.search);if(!keyValuePair||!keyValuePair[2]){return undefined}
...
_pushToHistory();target=targets.eq(targetIndex);$wrapper.trigger('next.ilb2',target);_loadImage(-1)},activityIndicatorOn=function(){$wrapper.append($activityObject)},activityIndicatorOff=function(){$('.imagelightbox-loading').remove()},overlayOn=function(){$wrapper.append($overlayObject)},closeButtonOn=function(){$buttonObject.appendTo($wrapper).on('click.ilb7',function(){_quitImageLightbox();return!1})},captionReset=function(){$captionObject.css('opacity','0');$captionObject.php('&nbsp;');if($(target).data('ilb2-caption')){$captionObject.css('opacity','1');$captionObject.php($(target).data('ilb2-caption'))}else if($(target).find('img').attr('alt')){$captionObject.css('opacity','1');$captionObject.php($(target).find('img').attr('alt'))}},navigationOn=function(){if(!targets.length){return}
